        ...
        <dailog size="small" :show.sync="myshow" classx="staff-add-user" :title="title" @ok="quire" :hide="(detailUser.type=='detail')">
            <div class="ui-table" slot="content">
                ...
                <table width="100%">
                    <tbody>
                        <tr>
                            <td class="table_name">客户编号</td>
                            <td>{{msgx.enterpriseNo}}</td>
                        </tr>
                        <tr>
                            <td class="table_name">客户类型</td>
                        ...
                        <tr>
                            <td class="table_name">客户性质</td>
                            <td>{{msgx.nature}}</td>
                        </tr>
                        <tr>
                            <td class="table_name">客户证照</td>
                            <td>
                        ...
                        <tr>
                            <td class="table_name">SCM</td>
                            <td>{{(msgx.enableScm == '1' ? '已启用' : '未启用')}}</td>
                        </tr>
                        <tr>
...
<script>
const URL = {
    DETAIL: 'scm.platformCustomer.findEnterprisePlatformByNo', // 详情
    CHECK: 'scm.platformCustomer.auditEnterprise' // 审核
};
import { formatFile } from '~/global.common';
    ...
    methods: {
        ...
        async getData () {
            await this.Http(URL.DETAIL, {
                ...
                re.data.nature = re.data.nature === '1' ? '公立医院' : re.data.nature === '0' ? '民营医院' : re.data.nature === '2' ? '检验子公司' : '';
                re.data.enterpriseType = re.data.scmEnterpriseType === 20 ? '合作共建' : re.data.scmEnterpriseType === 21 ? '非合作共建' : re.data.scmEnterpriseType === 22 ? '检验子公司' : '';
                re.data.auditStatusx = this.auditStatus(re.data.auditStatus);
                re.data.enterpriseCerts && (re.data.enterpriseCerts = re.data.enterpriseCerts.map(item => {
                    this.forms.ids.push(item.id);
                    // 图片
                    if (item.fileUrl) {
                        item.fileUrl_format = formatFile(item.fileUrl);
                        item.fileUrl = process.env.IMAGE_DOWNLOAD + item.fileUrl;
                    } else {
